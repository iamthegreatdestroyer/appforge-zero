...
const file = await googleDrive.uploadFile('/local/file.zip', '/remote/file.zip');
```
...
// Process payment
const payment = await paymentMgr.getProcessor('stripe').processPayment(99.99, 'usd');
...
const results = await manager.processPaymentMulti(
  amount,
  'usd',
  { productId, email }
);
...
storageManager.registerProvider(
  new DropboxProvider(dropboxToken)
);
...
for (const [provider, result] of syncResults) {
  console.log(`${provider}: ${result.filesUploaded} files, ${result.bytesSynced} bytes`);
}
```
