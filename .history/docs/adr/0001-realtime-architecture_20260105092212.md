...
**⚠️ IMPORTANT**: This ADR is for Phase 8B cloud infrastructure (IF Phase 8A is profitable).  
**Phase 8A** (Current): Desktop application = no real-time needed (see ALIGNMENT_CORRECTION_SUMMARY.md)  
**Status**: Only implement if Phase 8A revenue >$5K/month triggers Phase 8B.
...
Phase 8A delivers a desktop application MVP. Phase 8B (if profitable) will add cloud real-time functionality (WebSockets, notifications, activity streams, analytics) that must integrate with the auth layer, scale, be observable, and support automated deployment and compliance checks.
...
_Records:_ ADR created by GitHub Copilot on request of maintainer for Phase 8 kickoff.
