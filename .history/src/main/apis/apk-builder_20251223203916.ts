...
export class ApkBuilder {
  ...
  private createProjectStructure(config: ApkBuildConfig): string {
    ...
    // Copy app icon
    const iconPath = path.join(projectPath, 'app/src/main/res/drawable/ic_launcher.png');
    fs.copyFileSync(config.appIconPath, iconPath);

    // Create strings.xml
    const stringsPath = path.join(projectPath, 'app/src/main/res/values/strings.xml');
  ...
  private async signApk(
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        reject(new Error('jarsigner not found. Please install Java Development Kit.'));
        return;
      }
      ...
      const jarsignerProcess = spawn('jarsigner', [
        ...
        '-digestalg', 'SHA-256',
        '-keystore', signingKey.keyStorePath,
        '-storepass', signingKey.keyStorePassword,
        '-keypass', signingKey.keyPassword,
        apkPath,
        signingKey.keyAlias,
      ]);
      ...
      jarsignerProcess.on('close', (code) => {
        if (code === 0) {
          ...
          try {
            execSync(
              `jarsigner -verify -verbose -certs "${signedApkPath}"`,
              { stdio: 'ignore' }
            );
