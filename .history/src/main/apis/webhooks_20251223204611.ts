...
export class StripeWebhookHandler {
  ...
  private setupHandlers(): void {
    ...
    this.processor.onEvent(
      'payment.completed',
      async (payload) => this.handlePaymentCompleted(payload)
    );

    // Payment intent failed
    this.processor.onEvent(
      'payment.failed',
      async (payload) => this.handlePaymentFailed(payload)
    );

    // Charge refunded
    this.processor.onEvent(
      'payment.refunded',
      async (payload) => this.handlePaymentRefunded(payload)
    );
  }
  ...
  private async handlePaymentFailed(
    ...
    console.error(
      `Payment failed: ${intentId}, Reason: ${failureReason}`
    );
...
export class GumroadWebhookHandler {
  ...
  private setupHandlers(): void {
    this.processor.onEvent(
      'payment.completed',
      async (payload) => this.handleSaleCreated(payload)
    );
  }
...
export class KoFiWebhookHandler {
  ...
  private setupHandlers(): void {
    this.processor.onEvent(
      'payment.completed',
      async (payload) => this.handlePaymentCreated(payload)
    );
  }
...
export class ItchWebhookHandler {
  ...
  private setupHandlers(): void {
    this.processor.onEvent(
      'payment.completed',
      async (payload) => this.handleDownloadCreated(payload)
    );
  }
  ...
  private async handleDownloadCreated(
    ...
    console.log(
      `Itch.io download: User ${user_id} purchased game ${game_id}`
    );
...
export class UnifiedWebhookHandler {
   ...
   */
  parseStripeWebhook(body: string, signature: string, secret: string): WebhookPayload | null {
    try {
   ...
   */
  parseGumroadWebhook(body: string, signature: string, secret: string): WebhookPayload | null {
    try {
   ...
   * Parse Ko-fi webhook
   */
  parseKoFiWebhook(body: string, signature: string, secret: string): WebhookPayload | null {
    try {
   ...
   */
  parseItchWebhook(body: string, signature: string, secret: string): WebhookPayload | null {
    try {
