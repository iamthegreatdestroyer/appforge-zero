...
export class AuthenticationService {
    ...
    for (const config of mfaConfigs) {
      if (config.method === "totp" && config.secret) {
        ...
        if (verified) {
          ...
          await this.db.execute(`DELETE FROM mfa_sessions WHERE id = ?`, [
            sessionId,
          ]);
  ...
  async revokeSession(sessionId: string): Promise<void> {
    await this.db.execute(`UPDATE sessions SET is_active = 0 WHERE id = ?`, [
      sessionId,
    ]);
  }
  ...
  private async isLockedOut(email: string): Promise<boolean> {
    const results = await this.db.query(
      ...
      [
        email,
        new Date(Date.now() - this.config.loginAttemptLockoutDuration * 1000),
      ]
    );
  ...
  private async updateLastLogin(userId: string): Promise<void> {
    await this.db.execute(`UPDATE users SET last_login_at = ? WHERE id = ?`, [
      new Date(),
      userId,
    ]);
  }
