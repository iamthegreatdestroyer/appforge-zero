...
export class AuthorizationService {
   ...
   * Create RBAC policy
   */
  async createPolicy(policy: Omit<RBACPolicy, 'id' | 'createdAt'>): Promise<RBACPolicy> {
  ...
  async addPermissionToRole(
    ...
    await this.db.execute(
      `UPDATE roles SET permissions = ? WHERE role = ?`,
      [roleDef.permissions.join(','), role]
    );
  }
 ...
 */
export function createAuthorizationService(db: DatabaseManager): AuthorizationService {
