...
describe("Refresh Token Management", () => {
  ...
  test("should revoke refresh token", async () => {
    ...
    const valid = await refreshTokenManager.validateRefreshToken(
      token.token,
      "user_123"
    );
  ...
  test("should revoke all user tokens", async () => {
    ...
    const valid1 = await refreshTokenManager.validateRefreshToken(
      token1.token,
      "user_123"
    );
    const valid2 = await refreshTokenManager.validateRefreshToken(
      token2.token,
      "user_123"
    );
...
describe("RBAC Authorization", () => {
  ...
  test("should check user lacks admin permission", async () => {
    ...
    const hasPermission = await authService.hasPermission(user, "admin.access");
