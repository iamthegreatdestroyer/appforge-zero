...
describe("Database Layer", () => {
  describe("SQLiteDatabase", () => {
    ...
    it("should query data", async () => {
      ...
      await db.execute("INSERT INTO test_data (name, value) VALUES (?, ?)", [
        "item1",
        42.5,
      ]);

      const result = await db.query<any>("SELECT * FROM test_data");
    ...
    it("should support transactions", async () => {
      ...
      try {
        await db.transaction(async (transDb) => {
          await transDb.execute("INSERT INTO trans_test (value) VALUES (?)", [
            "first",
          ]);
