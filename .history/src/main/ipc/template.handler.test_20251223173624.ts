...
describe('TemplateIPCHandler', () => {
  ...
  describe('template:list', () => {
    it('should list all templates without filters', async () => {
      ...
      const response = await TemplateIPCHandler['handleList'](mockEvent, request);
    ...
    it('should sort templates by name', async () => {
      ...
      if (!('error' in response)) {
        ...
        for (let i = 1; i < templates.length; i++) {
          expect(templates[i].title.localeCompare(templates[i - 1].title)).toBeGreaterThanOrEqual(0);
        }
      }
    });
  ...
  describe('template:get', () => {
    it('should retrieve a valid template', async () => {
      ...
      const response = await TemplateIPCHandler['handleGet'](mockEvent, request);
