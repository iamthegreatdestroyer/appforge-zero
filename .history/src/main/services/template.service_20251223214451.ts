...
export class TemplateService extends BaseService {
   ...
   * Create template
   */
  async createTemplate(data: Partial<Template>): Promise<OperationResult<Template>> {
   ...
   * Update template
   */
  async updateTemplate(id: string, updates: Partial<Template>): Promise<OperationResult<Template>> {
    return this.executeOperation('updateTemplate', async () => {
      ...
      // Publish event
      await this.eventPublisher.publish('template.updated', 'template-service', {
        templateId: id,
  ...
  async deleteTemplate(id: string): Promise<OperationResult<void>> {
    return this.executeOperation('deleteTemplate', async () => {
      ...
      // Publish event
      await this.eventPublisher.publish('template.deleted', 'template-service', {
        templateId: id,
      });
  ...
  async archiveTemplate(id: string): Promise<OperationResult<Template>> {
    return this.executeOperation('archiveTemplate', async () => {
      ...
      // Publish event
      await this.eventPublisher.publish('template.archived', 'template-service', {
        templateId: id,
      });
   ...
   */
  async getTrendingTemplates(limit: number = 20): Promise<OperationResult<Template[]>> {
   ...
   * Add template review
   */
  async addReview(templateId: string, rating: number, comment: string): Promise<OperationResult<void>> {
 ...
 */
export function createTemplateService(ctx: ServiceContext, eventBus?: EventBus): TemplateService {
