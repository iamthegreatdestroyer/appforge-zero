...
class TrendAnalyzer implements TrendAnalyzerService {
  ...
  async scan(options: ScanOptions): Promise<Trend[]> {
    ...
    for (const [trendId, trend] of TREND_DATABASE) {
      ...
      // Filter by source
      if (!sources.includes(trend.source as any) && trend.source !== 'combined') {
        continue;
      }
  ...
  async listTrends(filters: TrendFilters): Promise<Trend[]> {
    ...
    // Filter by source
    if (source) {
      trends = trends.filter((t) => t.source === source || source === 'combined');
    }
  ...
  async suggestApps(trend: Trend): Promise<AppSuggestion[]> {
    ...
    const keywordToTemplates: Record<string, Array<{ templateId: string; name: string }>> = {
      'AI companion apps': [
  ...
  private calculateOpportunityScore(trend: Trend): number {
    ...
    const totalScore = volumeScore + growthScore + velocityScore + sentimentScore;
