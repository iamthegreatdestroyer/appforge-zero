...
const TrendList: React.FC<TrendListProps> = ({
  ...
  const getVelocityIndicator = (velocity: number): JSX.Element => {
    if (velocity > 0.1) {
      return (
        <span className="trend-list__indicator trend-list__indicator--up">
          📈
        </span>
      );
    }
    if (velocity < -0.1) {
      return (
        <span className="trend-list__indicator trend-list__indicator--down">
          📉
        </span>
      );
    }
    return (
      <span className="trend-list__indicator trend-list__indicator--stable">
        →
      </span>
    );
  };
