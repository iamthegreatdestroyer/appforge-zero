...
test.describe('Build Queue Management', () => {
  ...
  test('should open add to queue dialog', async () => {
    ...
    await expect(page.locator('[data-testid="add-queue-dialog"]')).toBeVisible();
  });
  ...
  test('should list available instances in dialog', async () => {
    ...
    await expect(page.locator('[data-testid="queue-instance-list"]')).toBeVisible();
  });

  test('should close add to queue dialog', async () => {
    await page.click('[data-testid="close-dialog"]');
    await expect(page.locator('[data-testid="add-queue-dialog"]')).not.toBeVisible();
  });
});
...
test.describe('Build Progress Monitoring', () => {
  ...
  test('should show build progress indicator', async () => {
    ...
    const progressExists = await page.locator('[data-testid="build-progress"]').count() > 0;
  ...
  test('should allow canceling a build', async () => {
    const activeBuilds = await page.locator('[data-testid="active-build"]').count();
    ...
    if (activeBuilds > 0) {
      ...
      await expect(page.locator('[data-testid="build-cancelled"]')).toBeVisible();
    }
  });
});
...
test.describe('Build History', () => {
  ...
  test('should show build log option', async () => {
    ...
    if (historyItems > 0) {
      await page.click('[data-testid="history-item"]:first-child');
      await expect(page.locator('[data-testid="view-build-log"]')).toBeVisible();
    }
  });
  ...
  test('should allow retrying failed builds', async () => {
    ...
    const failedBuilds = await page.locator('[data-testid="history-item"]').count();
  ...
  test('should open APK location for successful builds', async () => {
    ...
    const successBuilds = await page.locator('[data-testid="history-item"]').count();
    
    if (successBuilds > 0) {
      await page.click('[data-testid="history-item"]:first-child');
      await expect(page.locator('[data-testid="open-apk-location"]')).toBeVisible();
    }
  });
});
