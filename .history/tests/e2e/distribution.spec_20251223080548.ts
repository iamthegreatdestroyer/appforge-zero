...
test.describe('Distribution Channels', () => {
  ...
  test('should display distribution page', async () => {
    await expect(page.locator('[data-testid="page-distribution"]')).toBeVisible();
  });
  ...
  test('should open channel settings', async () => {
    await page.click('[data-testid="channel-gumroad"] [data-testid="channel-settings"]');
    await expect(page.locator('[data-testid="channel-settings-dialog"]')).toBeVisible();
  });
  ...
  test('should save API key for channel', async () => {
    ...
    await expect(page.locator('[data-testid="channel-settings-dialog"]')).not.toBeVisible();
  });
});
...
test.describe('Publishing Wizard', () => {
  ...
  test('should list publishable apps', async () => {
    ...
    await expect(page.locator('[data-testid="publishable-apps-list"]')).toBeVisible();
  });
  ...
  test('should proceed to channel selection', async () => {
    ...
    if (apps > 0) {
      ...
      await expect(page.locator('[data-testid="wizard-step-channels"]')).toBeVisible();
    }
  });

  test('should select channels for publishing', async () => {
    const onChannelStep = await page.locator('[data-testid="wizard-step-channels"]').isVisible();
  ...
  test('should set app price', async () => {
    const onPricingStep = await page.locator('[data-testid="wizard-step-pricing"]').isVisible();
  ...
  test('should close wizard on cancel', async () => {
    ...
    await expect(page.locator('[data-testid="publish-wizard"]')).not.toBeVisible();
  });
});
...
test.describe('Publications List', () => {
  ...
  test('should show publications section', async () => {
    await expect(page.locator('[data-testid="publications-section"]')).toBeVisible();
  });
  ...
  test('should filter publications by status', async () => {
    await page.selectOption('[data-testid="publication-status-filter"]', 'live');
  ...
  test('should open publication details', async () => {
    const publications = await page.locator('[data-testid="publication-item"]').count();
    
    if (publications > 0) {
      await page.click('[data-testid="publication-item"]:first-child');
      await expect(page.locator('[data-testid="publication-details"]')).toBeVisible();
    }
  });
});
