...
test.describe('Trend Scanning', () => {
  ...
  test('should open scan options dialog', async () => {
    ...
    await expect(page.locator('[data-testid="scan-options-dialog"]')).toBeVisible();
  });
  ...
  test('should show scan progress indicator', async () => {
    ...
    const scanningExists = await page.locator('[data-testid="scan-progress"]').count() > 0;
...
test.describe('Trend Details', () => {
  ...
  test('should show suggested templates', async () => {
    const detailVisible = await page.locator('[data-testid="trend-detail"]').isVisible();
  ...
  test('should allow creating app from trend', async () => {
    ...
    if (detailVisible) {
      await expect(page.locator('[data-testid="create-from-trend"]')).toBeVisible();
    }
  });
});
