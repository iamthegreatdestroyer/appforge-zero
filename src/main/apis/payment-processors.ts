...
export class StripeProcessor extends PaymentProcessor {
  ...
  async processPayment(
    ...
    const response = await this.client.post<any>("/payment_intents", {
  ...
  async refund(
    ...
    // Create refund
    const response = await this.client.post<any>("/refunds", {
  ...
  async listTransactions(
    ...
    const response = await this.client.get<any>("/payment_intents", {
      limit,
...
export class ItchProcessor extends PaymentProcessor {
  ...
  async getTransaction(externalId: string): Promise<PaymentTransaction> {
    // Fetch from itch.io
    const response = await this.client.get<any>(`/uploads/${externalId}`);
  ...
  async listTransactions(limit: number = 10): Promise<PaymentTransaction[]> {
    ...
    return response.sales
      ? response.sales.slice(0, limit).map((sale: any) => ({
          id: `itch-${sale.id}`,
          provider: "itch",
          externalId: sale.id,
          amount: sale.price,
