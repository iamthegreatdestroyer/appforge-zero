...
describe("BuildIPCHandler", () => {
  ...
  describe("build:create", () => {
    it("should create a build job", async () => {
      ...
      const response = await BuildIPCHandler["handleCreate"](
        mockEvent,
        request
      );
    ...
    it("should queue jobs sequentially", async () => {
      ...
      const response1 = await BuildIPCHandler["handleCreate"](
        mockEvent,
        request
      );
      const response2 = await BuildIPCHandler["handleCreate"](
        mockEvent,
        request
      );

      if (!("error" in response1) && !("error" in response2)) {
        expect((response1 as any).queuePosition).toBeLessThan(
          (response2 as any).queuePosition
        );
      }
    });
  });
  ...
  describe("build:queue", () => {
    it("should get queue position for valid job", async () => {
      ...
      const createResponse = await BuildIPCHandler["handleCreate"](
        mockEvent,
        createRequest
      );
      ...
      if (!("error" in createResponse)) {
        ...
        const queueResponse = await BuildIPCHandler["handleQueue"](
          mockEvent,
          queueRequest
        );
  ...
  describe("build:start", () => {
    it("should start a queued build", async () => {
      ...
      if (!("error" in createResponse)) {
        ...
        const startResponse = await BuildIPCHandler["handleStart"](
          mockEvent,
          startRequest
        );
  ...
  describe("build:cancel", () => {
    it("should cancel a running build", async () => {
      ...
      if (!("error" in createResponse)) {
        ...
        const cancelResponse = await BuildIPCHandler["handleCancel"](
          mockEvent,
          cancelRequest
        );
    ...
    it("should reject non-existent job", async () => {
      ...
      const response = await BuildIPCHandler["handleCancel"](
        mockEvent,
        request
      );
  ...
  describe("build:logs", () => {
    it("should retrieve logs for a job", async () => {
      ...
      if (!("error" in createResponse)) {
        ...
        const logsResponse = await BuildIPCHandler["handleLogs"](
          mockEvent,
          logsRequest
        );
