...
describe('TrendIPCHandler', () => {
  ...
  describe('trend:list', () => {
    ...
    it('should sort by volume descending', async () => {
      const request: IPC_TrendListRequest = { sortBy: 'volume', sortOrder: 'desc' };
    ...
    it('should sort by velocity ascending', async () => {
      const request: IPC_TrendListRequest = { sortBy: 'velocity', sortOrder: 'asc' };
      ...
      if (!('error' in response)) {
        const trends = (response as any).trends;
        for (let i = 1; i < trends.length; i++) {
          expect(trends[i].velocity).toBeGreaterThanOrEqual(trends[i - 1].velocity);
        }
      }
    });
  ...
  describe('trend:insights', () => {
    it('should generate insights for valid trend', async () => {
      ...
      const response = await TrendIPCHandler['handleInsights'](mockEvent, request);
  ...
  describe('trend:archive', () => {
    it('should archive trends', async () => {
      ...
      const response = await TrendIPCHandler['handleArchive'](mockEvent, request);
