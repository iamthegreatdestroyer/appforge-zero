...
class BuildPipeline implements BuildPipelineService {
  ...
  async listBuilds(
    ...
    builds.sort((a, b) => {
      switch (sortBy) {
        ...
        default:
          return (
            (b.startTime || b.endTime || new Date()).getTime() -
            (a.startTime || a.endTime || new Date()).getTime()
          );
      }
    });
  ...
  async signAPK(
    ...
    console.log(
      `[BuildPipeline] Using keystore: ${signingConfig.keystorePath}`
    );
  ...
  async getBuildStats(appId: string): Promise<{
    ...
    const durations = builds
      ...
      .map(
        (b) => (b.endTime!.getTime() - b.startTime!.getTime()) / 1000 // seconds
      );
