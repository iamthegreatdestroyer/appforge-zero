...
class MorphEngine implements MorphEngineService {
  ...
  async transformCharacters(input: MorphInput): Promise<MorphOutput> {
    ...
    for (const [charKey, morphRule] of Object.entries(morphRules.characters)) {
  ...
  async transformSetting(input: MorphInput): Promise<MorphOutput> {
    ...
    for (const [settingKey, morphRule] of Object.entries(morphRules.settings)) {
  ...
  private analyzeTransformationMetadata(
    ...
    const alteredElements = replacements.map((r) => r.original).slice(0, 10); // Top 10

    const estimatedQuality = Math.min(
      1,
      transformationScore *
        (1 - Math.abs(transformedWords - originalWords) / originalWords)
    );
